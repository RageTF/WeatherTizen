...
var inputData = function(data){
	...
	while(data.list.length-1>currWeather&&data.list[currWeather].dt*1000<=now){
			currWeather++;
	}
	$('#weather-list').append
	('<li id = "'+data.city.id+'">'+
	'<div class="w3-row w3-card-2 w3-round-small w3-border w3-border-blue">'+
		'<div id="'+data.city.id+'_div" class = " w3-panel w3-col s8">'+
		...
		'<div class = "w3-col s4 w3-white w3-display-container" align = "center">'+
			...
			'<h3 id="'+data.city.id+'_temp">'+Math.round(data.list[currWeather].main.temp-273.15)+' °C</h3>'+
			'<img id="'+data.city.id+'_img" src = "../css/images/'+data.list[currWeather].weather[0].icon + '.png">'+
		'</div>'+
	'</div>'+
	'</li>');
...
var refreshData = function(list){
	list.forEach(function(item, i , arr){
		...
		var now=new Date();
		now=now.getTime()-(1000*60*60*3);
		while((item.list.length-1>currWeather) && (item.list[currWeather].dt*1000<=now)){
				currWeather++;
		}
		$('#'+item.city.id+'_name').html(item.city.name);
		$('#'+item.city.id+'_temp').html(Math.round(item.list[currWeather].main.temp-273.15)+' °C');
		$('#'+item.city.id+'_img').attr('src','../css/images/'+item.list[currWeather].weather[0].icon + '.png');
...
var geolocationWeather = function() {
	if(navigator.geolocation){
		...
		$('#current-weather-container').html('<div class="w3-card-2 w3-blue w3-round-small w3-center w3-padding">'+ 
...
var findLocationSuccess = function(position) {
	...
	getCurrentWeatherByCoordinates(lat, lon, onGeolocationSuccess, onGeolocationError, onComplete);
}
