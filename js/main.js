...
var init = function () {
    ...
    $("#btn_search").bind("click", function(event, ui){
    	...
    	getWeatherByName(cityName,onSearchSuccess,onSearchError,onSearchComplete);
    });
...
var inputData = function(data){
	...
	'<div class="w3-row w3-card-2 w3-round-small w3-border w3-border-blue">'+
		'<div class = " w3-panel w3-col s8">'+
			'<h3 id="'+data.city.id+'_name">'+data.city.name+'</h3>'+
		'</div>'+
		'<div class = "w3-col s4 w3-white w3-display-container" align = "center">'+
			'<span id="'+data.city.id+'_btn" class="w3-closebtn w3-medium w3-display-topright w3-padding-small">&times;</span>'+
			'<h3 id="'+data.city.id+'_temp">'+Math.round(data.list[0].main.temp-273.15)+' °C</h3>'+
			'<img id="'+data.city.id+'_img" src = "http://openweathermap.org/img/w/'+data.list[0].weather[0].icon + '.png">'+
		'</div>'+
	'</div>'+
	'</li>');
...
var updateData = function(list){
	list.forEach(function(item, i ,arr){
		getWeatherById(item.city.id, onSearchUpdateSuccess, onSearchError, onSearchComplete);
	});
	getAndRefreshData();
}
...
var refreshData = function(list){
	list.forEach(function(item, i , arr){
		...
		$('#'+item.city.id+'_temp').html(Math.round(item.list[0].main.temp-273.15)+' °C');
		$('#'+item.city.id+'_img').attr('src','http://openweathermap.org/img/w/'+item.list[0].weather[0].icon + '.png');
		$('#'+item.city.id+'_btn').bind("click", function(event, ui){
			removeWeatherByCityId(item.city.id, onDeleteSuccess, onDeleteError);
		});
	});
}
...
var geolocationWeather = function() {
	if(navigator.geolocation){
		navigator.geolocation.getCurrentPosition(findLocationSuccess, findLocationError, options);
	}else{
		$('#main').append('<div id = "current-weather-container" class="w3-card-2 w3-blue w3-round-small w3-center w3-padding">'+ 
...
var findLocationSuccess = function(position) {
	...
	getWeatherByCoordinats(55.79, 49.12, onGeolocationSuccess, onGeolocationError, onComplete);
}

var findLocationError = function(error){
	$('#current-weather-container').append('<div id = "current-weather-container" class="w3-card-2 w3-blue w3-round-small w3-center w3-padding">'+ 
			'<h3>Sorry, but geolocation are not available</h3>'+ 
			'</div>');
}

var onGeolocationSuccess = function(data){
	$('#current-weather-container').append('<div id = "current-weather-container" class="w3-row w3-card-2 w3-blue w3-round-small">'+
	'<div class = " w3-panel w3-col s8">'+
		'<h3 id="geo_city"></h3>'+
	'</div>'+
...
'</div>')
	...
	var image="http://openweathermap.org/img/w/" + data.list[0].weather[0].icon + ".png";
